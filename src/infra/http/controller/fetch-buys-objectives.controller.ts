...
@Controller('/objectives/buys')
export class FetchBuysObjectivesController {
  constructor(private fetchBuysObjectives: FetchBuysObjectivesUseCase) {}
  ...
  async handle(
    ...
    const result = await this.fetchBuysObjectives.execute({
      authorId: userId,
      page,
    })
    ...
    const buys = result.value.buys

    return { buys: buys.map(ObjectivesPresenter.toHTTP) }
  }
}
