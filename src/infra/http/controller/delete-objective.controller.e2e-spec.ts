...
describe('Delete Objective (E2E)', () => {
  ...
  test('[DELETE] /objective/:objectiveId', async () => {
    ...
    const response = await request(app.getHttpServer())
      .delete(`/objective/${objective.id.toString()}`)
      .set('Authorization', `Bearer ${accessToken}`)
      .send()

    expect(response.statusCode).toBe(204)

    const objectiveOnDatabase = await prisma.objective.findUnique({
      where: {
        id: objective.id.toString(),
      },
    })

    expect(objectiveOnDatabase).toBeNull()
  })
})
